...
try:
    ...
    from torchvision.models.resnet import model_urls as _legacy_model_urls
except ImportError:
    _legacy_model_urls = None
...
def _get_resnet50_pretrained_url():
    if ResNet50_Weights is not None:
        return ResNet50_Weights.IMAGENET1K_V1.url
    if _legacy_model_urls is not None and 'resnet50' in _legacy_model_urls:
        return _legacy_model_urls['resnet50']
    raise RuntimeError('Unable to resolve pretrained ResNet50 URL for this torchvision version.')
...
def pdresnet50(pretrained=False, **kwargs):
    ...
    # pretrained = False
    if pretrained:
        local_template_ckpt = model_urls['pdresnet50']
        if os.path.exists(local_template_ckpt):
            state_dict_pdresnet50 = torch.load(local_template_ckpt)['state_dict']
            template_keys = list(state_dict_pdresnet50.keys())
        else:
            # Fallback for environments where the legacy local checkpoint file is absent.
            # We can still initialize from torchvision ResNet-50 pretrained weights.
            template_keys = list(model.state_dict().keys())

        state_dict_resnet50 = model_zoo.load_url(_get_resnet50_pretrained_url())
        state_dict_new = {}

        for key in template_keys:
            new_key = key[7:] if key.startswith('module.') else key

            source_key_candidates = [new_key]
            if 'conv4' in new_key:
                source_key_candidates.append(new_key.replace('conv4', 'downsample.0'))
            if 'bn4' in new_key:
                source_key_candidates.append(new_key.replace('bn4', 'downsample.1'))

            for source_key in source_key_candidates:
                if source_key in state_dict_resnet50:
                    state_dict_new[new_key] = state_dict_resnet50[source_key].clone()
                    break
